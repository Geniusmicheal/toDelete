...
(function ($) {
  ...
  var elementor = 0;
  if (
    window.location.href.indexOf("https://www.heirsholdings.com/?elementor-preview=") > -1 ||
  ...
  /***************************
    ...
    ***************************/
  $(".mil-has-children > a").after(
    '<div class="arrow-dropdown"><i class="fa fa-chevron-down"></i></div>'
  );
    ...
    pagination: {
      ...
      renderBullet: function (index, className) {
        ...
        return (
          '<span class="' +
          className +
          '"><span class="mil-custom-dot mil-slide-' +
          (index + 1) +
  ...
  if ($("footer .mil-social-icons .fab.fa-twitter").length) {
    ...
    $("footer .mil-social-icons .fab.fa-twitter").append(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="#ffffff" d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"https://www.heirsholdings.com/></svg>'
    );
  }
    ...
    function loop() {
      ...
      if ($(".mil-main-transition > div > div.elementor > section").length) {
        var sections = document.querySelectorAll(
          ".mil-main-transition > div > div.elementor > section"
        );
      } else {
  ...
  if ($(".js-replace-two").length && !elementor && $(window).width() > 991) {
    ...
    var scroll_two =
      ...
      function (callback_two) {
        window.setTimeout(callback_two, 1000 / 60);
      };
    ...
    function loop_two() {
      var top = window.pageYOffset;
      if ($(".mil-main-transition > div > div.elementor > section").length) {
        var sections_two = document.querySelectorAll(
          ".mil-main-transition > div > div.elementor > section"
        );
      } else {
      ...
      if (replaceItem_two.length > 0) {
        // get top position of item from container, because image might not have loaded
        replaceItemTop_two = parseInt(
          replaceContainer_two[0].getBoundingClientRect().top
        );
      ...
      if (lastPosition_two == window.pageYOffset) {
        ...
        Array.prototype.forEach.call(sections_two, function (el, i) {
          ...
          // active section
          if (
            sectionTop_two <= replaceItemBottom_two &&
            sectionBottom_two > replaceItemTop_two
          ) {
            // check if current section has bg
            ...
            if (
              currentSection_two ||
              el.querySelectorAll(".mil-dark-bg").length
            ) {
              replaceContainer_two[0].classList.remove(
                "js-replace--reverse-two"
              );
            } else {
          ...
          // if active Section hits replace area
          if (
            replaceItemTop_two < sectionTop_two &&
            sectionTop_two <= replaceItemBottom_two
          ) {
            // animate only, if section background changed
            if (currentSection_two != lastSection_two) {
              document.documentElement.style.setProperty(
                "--replace-offset-two",
                (100 / replaceItemHeight_two) *
                  parseInt(sectionTop_two - replaceItemTop_two) +
                  "%"
              );
            }
          }
  ...
  if ($(".js-replace-three").length && !elementor && $(window).width() > 991) {
    ...
    var scroll_three =
      window.requestAnimationFrame ||
      function (callback_three) {
        window.setTimeout(callback_three, 1000 / 60);
      };
    ...
    function loop_three() {
      var top = window.pageYOffset;
      if ($(".mil-main-transition > div > div.elementor > section").length) {
        var sections_three = document.querySelectorAll(
          ".mil-main-transition > div > div.elementor > section"
        );
      } else {
      ...
      var replaceContainer_three =
        document.querySelectorAll(".js-replace-three");
      var replaceItem_three = document.querySelectorAll(
        ".js-replace__item-three"
      );

      if (replaceItem_three.length > 0) {
        // get top position of item from container, because image might not have loaded
        replaceItemTop_three = parseInt(
          replaceContainer_three[0].getBoundingClientRect().top
        );
        replaceItemHeight_three = replaceItem_three[0].offsetHeight;
        replaceItemBottom_three =
          replaceItemTop_three + replaceItemHeight_three;
      }
      ...
      if (lastPosition_three == window.pageYOffset) {
        ...
        Array.prototype.forEach.call(sections_three, function (el, i) {
          ...
          // active section
          if (
            sectionTop_three <= replaceItemBottom_three &&
            sectionBottom_three > replaceItemTop_three
          ) {
            // check if current section has bg
            ...
            // switch class depending on background image
            if (
              currentSection_three ||
              el.querySelectorAll(".mil-dark-bg").length
            ) {
              replaceContainer_three[0].classList.remove(
                "js-replace--reverse-three"
              );
            } else {
              replaceContainer_three[0].classList.add(
                "js-replace--reverse-three"
              );
            }
          }
          ...
          // if active Section hits replace area
          if (
            replaceItemTop_three < sectionTop_three &&
            sectionTop_three <= replaceItemBottom_three
          ) {
            // animate only, if section background changed
            if (currentSection_three != lastSection_three) {
              document.documentElement.style.setProperty(
                "--replace-offset-three",
                (100 / replaceItemHeight_three) *
                  parseInt(sectionTop_three - replaceItemTop_three) +
                  "%"
              );
            }
          }
